...
if fs.access("/etc/init.d/entware") then
    ...
    local aria2_webui = "<br>" ..
        font_apply .. [[打开AriNG远程WebUI" onclick="window.open('http://ariang.mayswind.net/latest')"/>&nbsp;&nbsp;&nbsp;]] ..
        font_apply .. [[打开WebUI-Aria2远程WebUI" onclick="window.open('http://webui-aria2.1ge.fun/')"/><br><br>]] ..
        font_apply .. [[打开AriNG本地WebUI]] .. font_op .. [[/ariang')"/>&nbsp;&nbsp;&nbsp;]] ..
        font_apply .. [[打开WebUI-Aria2本地WebUI]] .. font_op .. [[/webui-aria2')"/><br>]]
    ...
    local function init(...)
        ...
        for _, param in ipairs({...}) do
            if fs.access(string.format("/opt/etc/init.d/%s", param)) then
                exist = true
                break
            end
        end
    ...
    local function running(binary, port)
        local isInstalled = binary and sys.call(string.format("[ -x /opt/bin/%s ]", binary)) == 0
