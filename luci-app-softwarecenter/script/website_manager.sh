...
install_website() {
	# 通用环境变量获取
	get_env
	unset filelink name dirname port hookdir istar
	[ $2 ] && port=$2
...
web_installer() {
	...
	[ $istar ] && suffix="tar"
	echo_time "开始安装 $name"
	[ -d "/opt/wwwroot/$name" ] && rm -rf /opt/wwwroot/$name /opt/tmp/$name.$suffix /opt/tmp/$name.$suffix \
	/opt/etc/nginx/vhost/$name.conf && echo_time "已删除以前的 $name 文件"
