		...
		makeLogArea(logdataArray) {
			...
			if (logdataArray.length > 0) {
				...
				logdataArray.forEach((e, i) => {
					...
					let TimeStr = new Date(e[1]).toLocaleString("zh-CN", {timeZone: "Asia/Shanghai"});
					lines.push(
						`<tr class="tr log-${e[4] || 'empty'}"><td class="td left log-entry-text-nowrap" data-title="#">${e[0]}</td>` +
						((e[1]) ? `<td class="td left log-entry-time-cell" data-title="${_('Timestamp')}">${TimeStr}</td>` : '') +
				...
				logTable.append(
					E('tr', { 'class': 'tr table-titles' }, [
						...
						(logdataArray[0][4]) ?
							E('th', { 'class': 'th left log-entry-text-nowrap' }, _('Levels')) : '',
						(logdataArray[0][5]) ?
