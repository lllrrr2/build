...
try {
    ...
    console.warn("当前脚本未检测到，假设仪表板正在使用 /old 后缀运行")
}
...
function renderStreamedHosts(options) {
    let html = `<div class="info-item">数据库流式传输到此代理</div>`;
...
function renderMachines(machinesArray) {
    ...
    if (!found) {
        if (machines) {
            ...
            html += (
                `<div class="info-item">
                    <a href="https://github.com/netdata/netdata/tree/master/registry#registry" target="_blank">联系注册表失败</a>
                </div>`
            )
        }
...
function restrictMyNetdataMenu() {
    setMyNetdataMenu(`<div class="info-item" style="white-space: nowrap">
        <span>Please <a href="#" onclick="signInDidClick(event); return false">登录 netdata.cloud</a> to view your nodes!</span>
        <div></div>
    </div>`);
}
...
function renderMyNetdataMenu(machinesArray) {
    ...
    if (machinesArray == registryAgents) {
        console.log("从注册表渲染 my-netdata 菜单");
    } else {
            ...
            if (typeof data !== 'undefined' && data !== null && typeof data.machine_guid === 'string' && data.machine_guid === guid) {
                ...
                if (!gotoServerStop) {
                    ...
                    if (gotoServerMiddleClick) {
                        ...
                        document.getElementById('gotoServerResponse').innerHTML = '<b>打开新窗口 ' + NETDATA.registry.machines[guid].name + '<br/><a href="' + verifyURL(finalURL) + '">' + escapeUserInputHTML(url) + '</a></b><br/>(检查您的弹出窗口拦截器是否失败)';
                    } else {
                ...
                if (gotoServerValidateRemaining <= 0) {
                    gotoServerMiddleClick = false;
                    document.getElementById('gotoServerResponse').innerHTML = '<b>对不起！我找不到该服务器的任何可操作 URL</b>';
                }
            }
        });
    }, (id * 50) + penalty);
    ...
    if (!isSignedIn()) {
        ...
        setTimeout(function () {
            if (gotoServerStop === false) {
                document.getElementById('gotoServerResponse').innerHTML = '<b>添加了该机器的所有已知 URL。</b>';
    ...
    if (n !== '' && n.length === 36) {
        NETDATA.registry.switch(n, function (result) {
            if (result !== null) {
                ...
                document.getElementById('switchRegistryResponse').innerHTML = "<b>对不起！注册表拒绝了您的请求。</b>";
            }
        });
    } else {
    ...
    if (deleteRegistryUrl) {
        if (isSignedIn()) {
            deleteCloudAgentURL(deleteRegistryGuid, deleteRegistryUrl)
                .then((count) => {
                    if (!count) {
                        responseEl.innerHTML = "<b>抱歉，该命令被 netdata.cloud 拒绝！</b>";
                        return;
                    }
            ...
            NETDATA.registry.delete(deleteRegistryUrl, function (result) {
                if (result !== null) {
                    ...
                    responseEl.innerHTML = "<b>抱歉，该命令被注册服务器拒绝！</b>";
                }
            });
        }
    }
    ...
    NETDATA.alarms.get('all', function (data) {
        ...
        function alarm_to_html(alarm, full) {
            ...
            var action_buttons = '<br/>&nbsp;<br/>role: <b>' + alarm.recipient + '</b><br/>&nbsp;<br/>'
                + '<div class="action-button ripple" title="单击可将仪表板滚动到该警报的图表" data-toggle="tooltip" data-placement="bottom" onClick="scrollToChartAfterHidingModal(\'' + alarm.chart + '\', ' + alarm.last_status_change * 1000 + ', \'' + alarm.status + '\'); $(\'#alarmsModal\').modal(\'hide\'); return false;"><i class="fab fa-periscope"></i></div>'
                ...
                + '<div class="action-button ripple" title="单击可将此徽章的自动刷新 <code>embed</code> html 元素复制到剪贴板" data-toggle="tooltip" data-placement="bottom" onClick="clipboardCopyBadgeEmbed(\'' + badge_url + '\'); return false;"><i class="fas fa-copy"></i></div>';
            ...
            $('#alarms_log_table').bootstrapTable({
                ...
                columns: [
                    ...
                    },
                    {
                        field: 'duration',
                        title: '上次持续时间',
                        titleTooltip: '在此事件之前，警报处于先前状态的持续时间',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                    ...
                    },
                    {
                        field: 'updates_id',
                        title: '更新ID',
                        titleTooltip: '由于该事件而废弃的事件的唯一 ID',
                        formatter: function (value, row, index) {
                            void (row);
                            void (index);
                            return alarmid4human(value);
    ...
    $.ajax({
        ...
        .done(function (response) {
            $.ajax({
                ...
                .done(function (data) {
                    data = data.replace(/(\r\n|\n|\r| |\t)/gm, "");
                    versionLog('GCS 的最新夜间版本是 ' + data);
                    callback(data);
                })
                .fail(function (xhr, textStatus, errorThrown) {
                    versionLog('无法从 GCS 下载最新的夜间版本 ID！');
                    callback(null);
                });
        })
        .fail(function (xhr, textStatus, errorThrown) {
            versionLog('无法从 GCS 下载最新的夜间版本！');
            callback(null);
        });
}
