...
if (typeof netdataServer !== 'undefined') {
    ...
    if (s) {
        ...
        console.log('警告：无法检测 Netdata 服务器的 URL。');
...
NETDATA.errorCodes = {
    ...
    408: {message: "Netdata 服务器向 HELLO 发送无效响应", alert: false},
    ...
    415: {message: "Netdata 警报下载失败", alert: false},
    416: {message: "Netdata报警日志下载失败", alert: false},
        ...
        if (typeof this.scalableUnits[units] !== 'undefined') {
            ...
            if (desired_units === 'auto') {
                ...
                if (typeof this.scalableUnits[units][desired_units] !== 'undefined') {
                    ...
                    console.log('不支持从 ' +units.toString() + ' 到 '+desired_units.toString() + ' 的单位转换。');
                    switch_units_callback(units);
                    return function (value) {
                        return value;
                    };
            ...
            console.log(`单位 ${units.toString()} 不匹配的单位转换方法`);
            switch_units_callback(units);
            return function (value) {
                return value;
            };
...
NETDATA.localStorageGet = function (key, def, callback) {
    ...
    if (typeof ret === 'undefined' || ret === 'undefined') {
        console.log('localStorage：加载未定义的"' + key.toString() + '"作为值 ' + ret + ' 类型 ' + typeof (ret));
        ret = def;
    }
...
NETDATA.dygraphChartUpdate = function (state, data) {
    ...
    if (state.tmp.dygraph_force_zoom) {
        if (NETDATA.options.debug.dygraph || state.debug) {
            state.log('dygraphChartUpdate() 强制缩放更新');
        }
        ...
        if (NETDATA.options.debug.dygraph || state.debug) {
            state.log('dygraphChartUpdate() 松散更新');
        }
    } else {
        if (NETDATA.options.debug.dygraph || state.debug) {
            state.log('dygraphChartUpdate() 严格更新');
        }
        ...
        <span class="dygraph__history-tip-content">
          ...
          <br/>
           <a href="https://docs.netdata.cloud/docs/configuration-guide/#increase-the-metrics-retention-period" target=_blank>
             配置 Netdata 的<b>历史记录</b></a>
           或使用<a href="https://docs.netdata.cloud/database/engine/" target=_blank>数据库引擎</a>。
        </span>
    `;
        ...
        entries.forEach(function (entry) {
            ...
            if (entry.intersectionRatio > 0) {
                ...
                if (idx === -1) {
                    ...
                    state.log("直到现在还不可见，但已经在visible_targets中");
                }
            } else {
                idx = NETDATA.intersectionObserver.visible_targets.indexOf(state);
                if (idx !== -1) {
                    ...
                    state.log("可见，但在visible_targets中找不到");
                }
            }
        ...
        const genLabel = function (state, parent, dim, name, count) {
            ...
            if (user_id !== null) {
                ...
                if (user_element === null) {
                    state.log('无法找到具有 id 的元素： ' + user_id);
                }
            }
        ...
        if (this.hasLegend()) {
            ...
            if (NETDATA.options.current.legend_toolbox && this.library.toolboxPanAndZoom !== null) {
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_left).popover({
                        ...
                        title: '向左平移',
                        content: '将图表向左平移。您还可以使用鼠标或手指（在触摸设备上）<b>拖动它</b>。<br/><small>可以从设置中禁用帮助。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_reset).popover({
                        ...
                        title: '图表重置',
                        content: '将所有图表重置为默认的自动刷新状态。您还可以使用鼠标或手指（在触摸设备上）<b>双击</b>图表内容。<br/><small>可以从设置中禁用帮助。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_right).popover({
                        ...
                        title: '向右平移',
                        content: '将图表平移到右侧。您还可以使用鼠标或手指（在触摸设备上）<b>拖动它</b>。<br/><small>可以从设置中禁用帮助。</small>'
                    });
                }
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.toolbox_zoomin).popover({
                        ...
                        title: '图表放大',
                        content: '放大图表。您还可以按 SHIFT 并选择图表的某个区域，或者按 SHIFT 或 ALT 并使用鼠标滚轮或两指触摸板滚动来放大或缩小。<br/><small>可以从设置中禁用帮助。 </小>'
                    });
                }
            ...
            if (NETDATA.options.current.resize_charts) {
                ...
                if (NETDATA.options.current.show_help) {
                    $(this.element_legend_childs.resize_handler).popover({
                        ...
                        title: '图表调整大小',
                        content: '使用鼠标或手指（在触摸设备上）拖动该点，以垂直调整图表大小。您还可以<b>双击它</b>或<b>双击它</b>在两种状态之间重置：默认状态和适合所有值的状态。<br/><small>帮助可以从设置中禁用。</small>'
                    });
                }
            ...
            if (NETDATA.options.current.show_help) {
                $(content).popover({
                    ...
                    content: '您可以单击或点击值或标签来选择尺寸。通过按 SHIFT 或 CONTROL，您可以启用或禁用多个维度。<br/><small>可以从设置中禁用帮助。</small>'
                });
            }
        } else {
            this.element_legend_childs = {
