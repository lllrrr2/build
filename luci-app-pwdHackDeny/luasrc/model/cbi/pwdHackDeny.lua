...
if (luci.sys.call("pidof pwdHackDeny.sh > /dev/null") == 0) then
	state_msg = [[<b><font color="green">]] .. translate("正在运行") .. [[</font></b>]]
else
	state_msg = [[<b><font color="red">]] .. translate("没有运行") .. [[</font></b>]]
end

if (luci.sys.call("[ `grep -c 'Bad password attempt' /etc/pwdHackDeny/badip.log.ssh 2>/dev/null` -gt 0 ]") == 0) then
	state_msg1 = "有SSH异常登录！"
else
	state_msg1 = "SSH最近登录日志"
end

if (luci.sys.call("[ `grep -c 'failed login on' /etc/pwdHackDeny/badip.log.web 2>/dev/null` -gt 0 ]") == 0) then
	state_msg2 = "有WEB异常登录！"
else
	state_msg2 = "WEB最近登录日志"
end
